...
```csv
time_ms,target_yaw_deg,target_pitch_deg,distance_m,cmd_yaw_deg,cmd_pitch_deg,state,control,fire,serial_on,serial_opened,send_ok,yaw_response_ms,pitch_response_ms
```

可以直接使用仓库里的脚本生成曲线图：

```bash
python3 tools/plot_telemetry.py build/logs/telemetry_YYYYMMDD_HHMMSS.csv
```

如果不传 CSV 路径，脚本会自动选择 `build/logs/` 中最新的一份遥测日志：
...
```text
build/logs/plots_telemetry_YYYYMMDD_HHMMSS/
├── index.html
├── yaw.svg
├── pitch.svg
└── response.svg
...
双击打开 `index.html`，或用浏览器打开对应的 `.svg` 文件即可截图。`yaw.svg` 中会自动绘制 `target_yaw_deg` 和 `-cmd_yaw_deg`，方便同方向对比。
...
```text
response yaw=xxxms pitch=xxxms
```

其中 `*` 表示当前正在统计一次响应过程。响应时间定义为：

- 当 `abs(yaw_error)` 或 `abs(pitch_error)` 大于 `5 deg` 时，开始计时。
- 当误差小于 `1 deg` 并连续保持 `10` 帧时，认为跟踪稳定。
- `response_ms = 稳定时间 - 开始时间`。

按 `p` 可以保存当前调试画面截图：

```text
logs/screenshot_YYYYMMDD_HHMMSS.png
```

注意：Arduino Serial Plotter 不能和主程序同时打开同一个云台控制串口。如果需要实时串口 Plotter，建议使用第二个 USB-TTL 专门输出调试数据；当前版本默认采用 CSV 日志，避免抢占云台串口。
